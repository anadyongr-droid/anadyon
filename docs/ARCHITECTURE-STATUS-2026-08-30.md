...
- **There is no staging database**, and preview deployments appear to carry the
  ...
  **Closed the same day** by splitting `SUPABASE_SERVICE_ROLE_KEY` and giving
  Preview a placeholder value — and confirmed at the behaviour, not only in
  the settings. The preview builds (Vercel reported Ready after the change) and
  reaches no data: a quote run against the preview returns "Price estimate
  unavailable", which is `/api/quote` failing to read `rates` through
  `supabaseAdmin` with the placeholder key. Both halves are now observed. The two `NEXT_PUBLIC_` variables were left as
