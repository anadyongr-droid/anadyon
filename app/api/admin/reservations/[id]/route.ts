...
export async function GET(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabaseAdmin
    ...
    // in as a prop.
    .select("*, vehicles(name, plate, category), quotes(ref, selected_model, vehicle_type, pricing_group, transmission, driver_age, baby_seat, child_seat, fdw, additional_drivers, comments, created_at, pickup_date, pickup_time, dropoff_date, dropoff_time, rental_days)")
    .eq("id", id)
    .single();
