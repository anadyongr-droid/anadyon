...
| E4 | **SUSPENDED — verify before changing anything.** This item said the root SPF authorises no `amazonses.com` so every booking confirmation hard-fails SPF. That conflated the `From:` header with the envelope sender; **SPF is checked against the Return-Path**, and the evidence says Resend's Return-Path is `send.anadyon.gr`, which already has `v=spf1 include:amazonses.com ~all` and the SES feedback MX. If so, nothing is broken and no change is needed. **One check settles it:** open a booking confirmation → show original → read `Return-Path:` and `Authentication-Results:`. `@send.anadyon.gr` with `spf=pass` closes this item; `@anadyon.gr` with `spf=fail` reopens it. **Do not add `include:amazonses.com` to the root meanwhile** — it authorises the entire shared SES pool to send as `anadyon.gr`. Reasoning in [`EMAIL-DELIVERABILITY.md`](EMAIL-DELIVERABILITY.md) and blueprint §10, 19 September. | **Tasos** (the one check) |
| E9 | **Decide whether to tighten DMARC from `p=none`.** Premature today and deliberately not done: the office relay path authenticates by SPF, but whether it is DKIM-signed is unverified, and tightening while a live sending path is misaligned puts staff mail in spam. The evidence is already being collected — `rua=mailto:customerservice@anadyon.gr` means aggregate XML reports arrive in that mailbox daily. Forward a week of them and an agent reads which sources pass and fail, then says whether `quarantine` is safe. | **Tasos** (forward the reports) |
