...
describe("finalising", () => {
  ...
  it("chooses the function from the row's direction, not from the request", async () => {
    ...
    expect(state.rpc[0].fn).toBe("finalise_check_in");
  });

  it("calls the check-out function for an outbound handover", async () => {
    state.handover = { id: HANDOVER, direction: "out" };
    await finalise();
    expect(state.rpc[0].fn).toBe("finalise_check_out");
  });

  it("leaves actor identity to the database gateway", async () => {
    await finalise();
    expect(state.rpc[0].args).not.toHaveProperty("p_actor");
    expect(state.rpc[0].args).not.toHaveProperty("p_actor_name");
  });
...
describe("voiding", () => {
  ...
  it("passes the trimmed reason through the identity-verifying gateway", async () => {
    await doVoid({ reason: "  started on the wrong car  " });
    expect(state.rpc[0].fn).toBe("void_handover");
...
describe("correcting", () => {
  ...
  it("checks the shape of a value but leaves the meaning to the database", async () => {
    ...
    expect(state.rpc[0].fn).toBe("correct_handover");
  });
