...
| E9 | **Decide whether to tighten DMARC from `p=none`.** Better news than expected: the 19 September header measurement shows **both** sending paths DKIM-signed and strictly aligned — Resend as `d=anadyon.gr` via the `resend` selector, and the Papaki relay as `d=anadyon.gr` via the `default` selector, both published and both validating. Still not sufficient to act on: the message measured was one Papaki *forwarded*, not one a staff member *originated*, and "almost certainly identical" is exactly how the E4 finding went wrong. **Forward a week of the `rua` aggregate reports** (XML attachments already arriving daily at `customerservice@anadyon.gr` from `noreply-dmarc-support@google.com`) and an agent reads which sources pass, including ones nobody has thought of. Then `quarantine`. [`EMAIL-DELIVERABILITY.md`](EMAIL-DELIVERABILITY.md). | **Tasos** (forward the reports) |
