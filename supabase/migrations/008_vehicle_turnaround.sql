...
-- car is not. Defaults below are starting points, adjustable per vehicle.
-- ============================================================================

ALTER TABLE vehicles
  ADD COLUMN IF NOT EXISTS turnaround_minutes integer NOT NULL DEFAULT 180;
...
-- Seed by category. Cars keep the 180-minute default.
UPDATE vehicles SET turnaround_minutes = 30
  WHERE pricing_group = 'bike' AND turnaround_minutes = 180;

UPDATE vehicles SET turnaround_minutes = 60
  WHERE pricing_group IN ('motorbike_a','motorbike_b') AND turnaround_minutes = 180;
