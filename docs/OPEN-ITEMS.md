...
| R2 | **Dependabot backlog — worked through 20 Sep, six of seven resolved.** Security triage earlier that day merged #86, #108, #106, #103, #104 and #101; `npm audit` on `main` reports **0 vulnerabilities**, so nothing remaining is a security fix. The seven maintenance PRs were then assessed individually rather than merged to empty the queue, and **four were wrong as offered**: #83 bumped one of three CodeQL entry points that must share a tag; #85 offered `@types/node` ^26 against a Node 24 runtime; #78/#79/#80 named three node20 actions while a fourth, `actions/cache`, was node20 with no PR at all. Replaced by **#130, #131, #132 and #133**, each with the SHA resolved from the upstream tag and each target's `action.yml` read at that SHA. `lib/actionPinParity.test.ts` now fails on a split pin. **#87 (TypeScript 7.0.2) remains open and is blocked upstream, not by us** — `tsc` passes; `typescript-eslint` refuses to load against TS 7 and caps at `<6.1.0` two levels down through `eslint-config-next`. Tracked at typescript-eslint#10940; it becomes mergeable on its own. Do not close it. | Agent |
