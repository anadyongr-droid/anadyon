-- Provider-neutral ledger for NBG-hosted deposit payments.
--
-- Card data never reaches this table. It records only the bank order/session
-- references needed to reconcile a hosted payment with one reservation.
begin;
...
-- A double-click or two Vercel instances must not create two simultaneously
-- payable links for the same deposit. A completed/failed attempt releases the
-- slot so staff can issue a replacement.
...
-- One database transaction records the verified attempt and confirms the
-- reservation. The function is invoker-rights and callable only by the
-- service role, so it adds atomicity without adding a privilege-escalation
-- surface.
