-- Link every website request to one customer and its one operational
-- reservation.  The relationship is deliberate rather than inferred from an
-- email address or free-text note, so a repeat customer's requests stay
-- distinct and conversion reporting is reliable.
begin;
...
begin
  ...
  -- A customer record is part of the same transaction as the request.  An
  -- existing record keeps the staff-maintained values; a web request fills
  -- only missing contact details and records the latest interaction.
