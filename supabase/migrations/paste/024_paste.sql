-- 024_fix_create_web_booking_defaults.sql — paste this whole block into the Supabase SQL editor.
begin;
...
begin
  ...
  if p_idempotency_key is not null and length(trim(p_idempotency_key)) > 0 then
    ...
    if found then
      return jsonb_build_object(
        'ref', v_existing_ref, 'discount', coalesce(v_existing_discount, 0),
        'total', v_final_total, 'deposit', v_final_deposit,
        'balance_due', v_final_balance, 'idempotent_replay', true
      );
    end if;
  else
  ...
  p_quote := (p_quote - array['id', 'created_at']) || jsonb_build_object(
    'idempotency_key', p_idempotency_key, 'total', v_final_total,
  ...
  p_reservation := (p_reservation - array['id', 'created_at', 'updated_at']) || jsonb_build_object(
    'total', v_final_total, 'deposit', v_final_deposit,
    'balance_due', v_final_balance, 'discount_amount', v_discount,
  ...
  select string_agg(quote_ident(c.column_name), ', ' order by c.ordinal_position)
    into v_quote_cols from information_schema.columns c
  ...
  select string_agg(quote_ident(c.column_name), ', ' order by c.ordinal_position)
    into v_reservation_cols from information_schema.columns c
  ...
  return jsonb_build_object(
    'ref', v_ref, 'quote_id', v_quote_id, 'promo_id', v_promo_id,
    'discount', v_discount, 'total', v_final_total,
    'deposit', v_final_deposit, 'balance_due', v_final_balance,
