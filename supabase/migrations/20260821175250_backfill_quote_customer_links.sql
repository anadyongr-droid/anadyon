-- Backfill the existing website quotes after the structural/function change.
-- The matching key is normalised email, then the explicit legacy quote ref in
-- the reservation note.  No reservation is guessed from name or dates.
begin;
...
select distinct on (lower(trim(q.email)))
  ...
  nullif(concat_ws(' ', nullif(trim(q.first_name), ''), nullif(trim(q.last_name), '')), ''),
  lower(trim(q.email)),
  nullif(trim(q.mobile_tel), ''),
  nullif(trim(q.address), ''),
  nullif(trim(q.postal_code), ''),
  nullif(trim(q.city), ''),
  nullif(trim(q.country), ''),
  q.created_at
...
on conflict ((lower(email))) where email is not null do update
  set last_interaction_at = greatest(c.last_interaction_at, excluded.last_interaction_at),
      updated_at = now();
