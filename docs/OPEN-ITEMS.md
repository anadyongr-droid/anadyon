...
**Last verified:** 20 September 2026, Codex.
...
| R2 | **Dependabot backlog.** Security triage completed 20 Sep: #86, #108, #106, #103, #104 and #101 merged serially after the full current-main build, staging E2E, security analysis and Vercel checks passed. A fresh `npm audit` on `main` at `f310fbf` reports **0 vulnerabilities**. Remaining open updates are maintenance, not known audit fixes. In particular, grouped PR #122 also upgrades Stripe 22.5→22.6 and fails until the pinned API version is deliberately changed; do not treat that compatibility change as a mechanical fix. #87 still fails its build. Refresh and assess the remaining maintenance PRs individually; never bypass protection. | Agent |
